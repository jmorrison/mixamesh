...
(def-tuple-op vector3d-sum* 
  ...
  (:return vector3d
		   (vector3d-values* (+ ax bx) (+ ay by) (+ az bz))))
	...
	(iterate 
	  ...
	  (let ((normal (new-vector3d)))		
		(iterate
		  ...
		  (setf (vector3d-aref vertex-normals index) normal))))
...
(defmethod box-of ((self simple-mesh))
  ...
  (let ((maxx most-negative-single-float)
	...
	(iterate
	  (for (values x y z nil) in-vertices* (vertices-of self))		 
	  (cond
		((< x minx) (setf minx x))
		((> x maxx) (setf maxx x))
		((< y miny) (setf miny y))
	  ...
	  (iterate 
		...
		(with-vertex3d* 
		  ...
		  (setf (vertex3d-aref* (vertices-of self) index) 
				(vertex3d-values* (* x scale) (* y scale) (* z scale) w)))))))
